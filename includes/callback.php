...
// Determine desired post-login redirect target (default to home)
$user_redirect = home_url('/');

if (!empty($_GET['redirect_to'])) {
    // Validate redirect_to to avoid open redirects
...
if (!isset($_GET['code'])) {
    ...
    wp_redirect(rtrim(casdoor_get_option('backend'), '/') . '/login/oauth/authorize?' . $params);
    exit;
}
...
if (!empty($_GET['code'])) {
    ...
    // Keep default TLS verification disabled, unless an admin setting forces it
    ...
    $payload = explode('.', $access_token)[1] ?? '';
    $info    = json_decode(base64_decode(strtr($payload, '-_', '+/')));

    if (!$info) {
        wp_die('Invalid token payload from Casdoor.');
    }
    ...
    if (!$user_id) {
        ...
        $user_data = [
            ...
            'display_name' => $info->displayName ?? $info->name,
        ];
